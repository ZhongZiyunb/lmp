...
int VideoEncoder::find_resolution(uint32_t birate_kpbs)
{
	...
	logger->log_print(LOG_LEVEL::DEBUG, "%s %s resolution = %u, birate_kpbs = %u\n", __FILE__, __LINE__, ret, birate_kpbs);
	return ret;
}
...
void VideoEncoder::try_change_resolution()
{
	...
	if (res.min_rate > bitrate_kbps_ && curr_resolution_ > VIDEO_120P)
	{
		...
		if (rate_stat_kps < res.min_rate * 7 / 8) /*�������ݵĴ���С����С���ƴ����������Ķ�*/
			return;
	...
	else if (frame_index >= (KEY_FRAME_SEC * frame_rate_ / 2) && res.max_rate < bitrate_kbps_ && curr_resolution_ + 1 <= max_resolution_ && up_ts_ + 10000 < GET_SYS_MS())
	{
		/*����һ��ֱ���*/
		curr_resolution_ = find_resolution(bitrate_kbps_);
		close_encoder();
...
VideoDecoder::VideoDecoder()
{
	...
	// 测试 打开输出文件
	out_file = fopen("/mnt/d/output.yuv", "wb");
	if (!out_file)
	{
		logger->log_print(LOG_LEVEL::WARN, "%s \n", "open out file failed");
	}
}
...
bool VideoDecoder::decode(uint8_t *in, int in_size, uint8_t **out, int &out_width, int &out_height, int32_t &pict_type)
{
	...
	/*** 这边是 FFMPEG 4.3 版本的解码实现 该版本已经不支持avcodec_decode_video2()

		int res = avcodec_send_packet(de_context_, &avpkt_);
		if (res < 0) {
			char errbuf[1024];
		...
		while (true) {
			...
			ret = avcodec_receive_frame(de_context_, de_frame_);
			...
			if (out_file != NULL) {
				// 写入Y平面数据
				fwrite((const char *)de_frame_->data[0], 1, de_frame_->linesize[0] * de_frame_->height, out_file);
				// 写入U平面数据
				fwrite((const char *)de_frame_->data[1], 1, de_frame_->linesize[1] * de_frame_->height >> 1, out_file);
				// 写入V平面数据
				fwrite((const char *)de_frame_->data[2], 1, de_frame_->linesize[2] * de_frame_->height >> 1, out_file);
				logger->log_print(LOG_LEVEL::INFO,"%s | ret: %i \n","decode write",ret);
				cnt ++;
				fflush(out_file);
				// if (cnt == 300) {
				// 	fclose(out_file);
			...
			logger->log_print(LOG_LEVEL::INFO,"%s | ret: %i | cnt: %i\n","decode 1 ",ret, cnt);
			if (ret == 1) break;
			if (ret == 0) break;
	...
	*/
	...
	// 	sws_context_ = sws_getContext(curr_width_, curr_height_, AV_PIX_FMT_YUV420P,
	// 		curr_width_, curr_height_, AV_PIX_FMT_BGR24, SWS_FAST_BILINEAR, NULL, NULL, NULL);
	// }

	if (sws_context_ != NULL)
	{
		// @todo 解码
		// avpicture_fill(&outpic_, buff_, AV_PIX_FMT_RGB24, curr_width_, curr_height_);
		// //YUV -> RGB, No scale
