...
bool H265Encoder::open_encoder()
{
	...
	sws_context_ = sws_getContext(src_width_, src_height_,
		AV_PIX_FMT_BGR24, res.codec_width, res.codec_height,
...
bool H265Encoder::encode(uint8_t *in, int in_size, enum AVPixelFormat pix_fmt, uint8_t *out, int *out_size, int *frame_type, bool request_keyframe)
{
	int rc;
	bool ret = false;
