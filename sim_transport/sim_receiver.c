...
static int real_video_cache_get(sim_session_t* s, sim_frame_cache_t* c, uint8_t* data, size_t* sizep, uint8_t* payload_type, int loss)
{
	...
	*payload_type = 0;
	...
	sim_debug("state: %i |wait_timer: %d|frame_timer: %d| max fid: %d| min fid: %d|max ts: %d|\n",c->state,c->wait_timer,c->frame_timer,c->max_fid,c->min_fid,c->max_ts);
	...
	pos = INDEX(c->min_fid + 1);
	frame = &c->frames[pos];
	sim_debug("frame_ts: %d|c_frame_ts:%d|play_ready_ts:%d|space:%d\n",frame->ts,c->frame_ts,play_ready_ts,space);
	sim_debug("frame_fid:%d|is_frame_full:%i\n",frame->fid,real_video_cache_check_frame_full(s, frame));
	if ((c->min_fid + 1 == frame->fid || frame->frame_type == 1) && real_video_cache_check_frame_full(s, frame) == 0){
		/*���м�Ъ�Կ��*/
